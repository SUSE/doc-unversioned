...
{trento} must run on on a HANA database to be able to discover an {sap} system. In addition, the parameter `dbs/hdb/dbname` in the DEFAULT profile of the {sap} system must be set to the database (tenant) name as its value.
