...
NOTE: Skip this step on {slea}, as the correct configuration is already in place
====
...
NOTE: Do not remove existing entries in `transports.docker`. Append the entry for `registry.suse.com` to the list
====
...
NOTE: This step is optional on {sle}. The signing key is already available in `/usr/share/pki/containers/suse-container-key.pem`
====
