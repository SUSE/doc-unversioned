...
Linux containers offer a lightweight virtualization method to run multiple isolated environments simultaneously on a single host. The Linux kernel's namespaces and kernel control groups features enable the container to be isolated from the rest of the host system and other containers.
