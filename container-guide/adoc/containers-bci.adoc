...
Application {bcia}s are {bcia}-Base container images that include specific applications, such as the PostgreSQL database and the Grafana analytics and monitoring solution. Application {bcia}s are available in the dedicated section of https://registry.suse.com[{suseregistry}].
