...
.{bcirepo} repository
====
There is a {bcirepo} repository for each {slea} service pack. This means that {bcia}s based on SP4 have access to the {bcirepo} repository for SP4, all {bcia}s based on SP5 use the SLE_BCI repository for SP5, and so on. Each SLE_BCI repository contains all SLE packages except kernels, boot loaders, installers (including {yast}), desktop environments and hypervisors (such as KVM and Xen).
====

[NOTE]
.Requesting a missing package
====
If the {bcirepo} repository does not have a package you need, you have two options. As an existing {suse} customer, you can file a feature request. As a regular user, you can request a package to be created by creating an issue in https://bugzilla.suse.com/enter_bug.cgi?product=SUSE%20Linux%20Enterprise%20Base%20Container%20Images[Bugzilla].
====
...
Application {bcia}s are {bcia}-Base container images that include specific applications, such as the PostgreSQL database and the Performance Co-Pilot a system-level performance analysis toolkit. Application {bcia}s are available in the dedicated section of https://registry.suse.com/#apps[{suseregistry}].
...
com.suse.supportlevel::
  ...
  * `acc` Software delivered with the {bci} may require an external contract.
...
The default package manager in {slsa} is {zypper}. Similar to APT in Debian and APK in Alpine Linux, {zypper} offers a command-line interface for all package management tasks. Below is a brief overview of commonly used container-related {zypper} commands.
