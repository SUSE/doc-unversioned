...
To install {podman}, make sure you have the {slea} Containers Module enabled (see <<cha-sle-containers-module>>), run the command `sudo zypper in podman`. Then run `podman --version` to check whether {podman} has been installed successfully.
