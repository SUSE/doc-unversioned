...
This chapter provides a brief overview of tools, provided by SUSE, for building images and managing containers. All our tools mentioned below are part of the *{sles} {productnumber} Containers Module*, except the Open Build Service. You can see the full list of packages in the *Containers Module* in the https://scc.suse.com/packages[{scc}].
...
[[sec-tools-helm]]
==== {helm}
{helm} is the Kubernetes Package Manager, it manage charts, which are packages of pre-configured Kubernetes resources. Allowing to easily package, configure, and deploy applications and services onto Kubernetes clusters.
