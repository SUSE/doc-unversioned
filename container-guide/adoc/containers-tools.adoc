...
[[sec-tools-suse-registry]] === {suseregistry} https://registry.suse.com is the
only source for official {suse} base container images. It contains tested, up to
date and certified {slea} base container images. All images in the
{suseregistry} are regularly rebuilt to include updates and fixes. The
{suseregistry}'s Web user interface lists a subset of the available images: Base
Container Images, Language Container Images, Application Container Images,
{suse} {sles} Images, and Releases Out of General Support. The Web UI also
provides additional information for each image, including release date, support
level, size, digest of the images and packages inside the image.
...
tool to {podman}, and the `podman build` command uses {buildah} to perform container image builds.
...
{skopeo} is can inspect containers and repositories in remote or local
...
[[sec-tools-distribution]] === {distribution}
https://github.com/distribution/distribution[Distribution] is an open-source
registry implementation for storing and distributing container images using the
OCI Distribution Specification. It provides a simple, secure, and scalable base
for building a large scale registry solution or running a simple private
registry.
