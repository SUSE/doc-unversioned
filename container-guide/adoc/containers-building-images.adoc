...
Build the image by running the following command in the directory where the `Dockerfile` resides in:

....
> podman build --tag tux_application:latest .
....

The `Dockerfile` example above performs the following operations during the image build process:
...
[NOTE]
.Applying changes to the shared directory
====
