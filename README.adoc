...
* The documentation toolchain can be utilized from a container using
  daps2docker. To do this, ensure that the Documentation:Tools repo is configured on the system
  then install daps2docker and create a config file for daps2docker.
+
[source]
----
sudo addrepo https://download.opensuse.org/repositories/Documentation:/Tools/16.0/ Documentation:Tools
sudo zypper install daps2docker
----

** ensure that ~/.config/daps2docker/config contains
+
[source]
----
formats="html,pdf"
...
*  For a non-containerized install of the documentation environment use the following command:
+
[source]
----
...
* daps2docker usage:
** replace `daps` with `daps2docker` and run per above
