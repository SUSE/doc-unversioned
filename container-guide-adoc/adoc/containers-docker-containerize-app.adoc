...
The following section provides examples and recommendations on creating container images for applications. Before proceeding, make sure that you have activated your {sles} base image as described in link:#sec-download-prebuild-images[Pulling base {slsa} images].
...
<1> Pulls the `sle15` base image as described in link:#sec-download-prebuild-images[Pulling base {slsa} images].
...
<1> Pull the base image as described in link:#sec-download-prebuild-images[Pulling base {slsa} images].
...
.. Make sure the {slea} images from the {suseregistry} are installed as described in link:#sec-download-prebuild-images[Pulling base {slsa} images].
