...
====
Starting with {deng} 1.12, container orchestration is now an integral part of {deng}. Even though this feature is available in {sles} {productnumber}, it is not supported by SUSE and is only provided as a technology preview. Use K8S for container orchestration. For details, refer to the http://kubernetes.io/docs/getting-started-guides/kubeadm/[Kubernetes documentation].
====
...
Since {sles} 12 onward, the Btrfs file system is used by default, which forces {deng} to use the btrfs driver.
